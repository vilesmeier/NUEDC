...
void ADC_Get_Value(void)
{
    ...
    state.V_Input  = ((AdcRegs.ADCRESULT0 >> 4) * 0.0007326007326f - 1.49998f) * 31.0f;
    //ADCResult1 - Input     AC Current - B0
    state.I_Input  = ((AdcRegs.ADCRESULT1 >> 4) * 0.0007326007326f - 1.49998f) * 4.166667f;
    //ADCResult2 - Rectifier DC Voltage - A1
    state.V_DC     = ((AdcRegs.ADCRESULT2 >> 4) * 0.0007326007326f) * 31.0f;
    //ADCResult3 - Rectifier DC Current - B1
    state.I_DC     = ((AdcRegs.ADCRESULT3 >> 4) * 0.0007326007326f - 1.49998f) * 4.166667f;
}
...
interrupt void ADC_int_isr(void)
{
    ...
    if(state.Flag_Input_Enable == INPUT_ENABLE)
    {
        ...
        {
            state.V_Output_Softstart = state.V_Output_Softstart + 0.00035f;
        }

        if((state.V_Output_Softstart > state.V_DC_Set)&&(state.Flag_Output_SoftStart == SOFT_START))
        {
        ...
        if(state.Flag_Output_SoftStart == SOFT_START)
        {
            state.V_GiveValue    = state.V_Output_Softstart;
        }
        else if(state.Flag_Output_SoftStart == NORMAL)
        {
            state.V_GiveValue    = state.V_DC_Set;
        }

        //DC Voltage Loop
        PID_Run(&pi_DC, state.V_GiveValue, notch.result, 0);

        PR_Run(&pr_Input, pi_DC.voltage * rms_Vol_Input.angle, state.I_Input, -state.V_Input);

        EPwm1Regs.CMPA.half.CMPA = 3750 * (1 - pr_Input.out);
        EPwm2Regs.CMPA.half.CMPA = 3750 * (1 + pr_Input.out);

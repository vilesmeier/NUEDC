 ...
 *
 */
void EPWM_AQ_SoftForce_OFF(void)
{
    EALLOW;
    //Output A AQ Action
 ...
 * 
 */
void EPWM_AQ_SoftForce_LOW(void)
{
    EALLOW;
...
interrupt void epwm1_isr(void)
{
    ...
    POWER_3ph_Set_Voltage(&v, sinf(ramp.ref_angle), sinf(ramp.ref_angle - 0.66667f * PI), sinf(ramp.ref_angle + 0.66667f * PI));
    POWER_3ph_Set_Current(&v, sinf(ramp.ref_angle + 0.5f * PI), sinf(ramp.ref_angle - 0.66667f * PI + 0.5f * PI), sinf(ramp.ref_angle + 0.66667f * PI + 0.5f * PI));

    POWER_3ph_Run(&v);

    UART_VOFA_1Ch_ShowFloat(ramp.ref_angle);

...
{
    dq.cos = cosf(ramp.ref_angle);
    dq.sin = sinf(ramp.ref_angle);

    Inverse_Park_Run(&dq);
    Inverse_Clark_Run(&dq);

    SVPWM_Run(&svpwm, dq.alpha, dq.beta);

    Ramp_Run(&ramp);


    UART_VOFA_3Ch_ShowFloat(svpwm.comp1, svpwm.comp2, svpwm.comp3);

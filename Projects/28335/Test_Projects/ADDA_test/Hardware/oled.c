...
/**
  * @brief OLED初始化,使用28335片上的IIC外设，使能FIFO模式
  * 
  */
void OLED_Init(void)
{
    ...
    // 预分频——时钟模块的频率
    I2caRegs.I2CPSC.all = 12;       //预分频   IIC模块时钟需设置为7-12MHz，本实验设置为 (150/15 = 10MHz)
    I2caRegs.I2CCLKL = 20;   //时钟低电平时间值
    I2caRegs.I2CCLKH = 20;   //时钟高电平时间值
...
void OLED_ShowChar(Uint8 x,Uint8 y,Uint8 chr)
{
    ...
    for(i=0;i<6;i++)
    {
        WriteData(F8x6[c*6+i],OLED_DATA);
    }
}

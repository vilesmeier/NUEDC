...
{
    //A0 3PH voltag  -- ab
    state.V_ab  = (AdcMirror.ADCRESULT0 * 0.0007326007326f -1.499657842f) * 31.0f;
    //B0 3PH current -- A
    state.I_A  = (AdcMirror.ADCRESULT1 * 0.0007326007326f - 1.50627f) * 4.149549774f;
    //A1 3PH voltage -- bc
    state.V_bc  = (AdcMirror.ADCRESULT2 * 0.0007326007326f -1.49967871f) * 31.0f;
    //B1 3PH current -- B
    state.I_B  = (AdcMirror.ADCRESULT3 * 0.0007326007326f - 1.500183151f)* 4.231381197f;
    //A2 3PH voltage -- ca
    state.V_ca  = (AdcMirror.ADCRESULT4 * 0.0007326007326f -1.499347089f) * 31.0f;
    //B2 3PH current -- C

...
void PIR_Run(PIR *pir, float target, float actual, float feedforward)
{
    ...
    pir->output = pir->B0 * pir->err + pir->B1 * pir->e_last_1 + pir->B2 * pir->e_last_2 - pir->A1 * pir->Res_last_1 - pir->A2 * pir->Res_last_2;

    // update storage
    pir->e_last_1 = pir->err;
    pir->e_last_2 = pir->e_last_1;
    pir->Res_last_1 = pir->output;
    pir->Res_last_2 = pir->Res_last_1;

    // get output
    pir->nolimit = pir->Kp * pir->err + pir->integral * pir->Ki + pir->output + pir->feedforward;

 ...
 */
void POWER_1ph_Set(POWER *v, float isrFrequency, float threshold, float gridMaxFreq, float gridMinFreq)
{
 ...
 */
void POWER_1ph_Run(POWER *v, float vol, float cur)
{
    v->v = vol;
    v->i = cur;
    v->vNorm = fabsf(v->v);
 ...
 */
void POWER_RMS_1ph_Run(POWER *v,float vrms, float irms, float pf )
{
    v->vRms = vrms;
    v->iRms = irms;
    v->powerFactor = pf;

...
void ADC_Get_Value(void)
{
    ...
    state.I_out_A = (AdcMirror.ADCRESULT1 * 0.0007326007326f - 1.5069f) * 3.000939644f;

    // A1 3PH voltage -- Inverter Ubc
    state.V_bc    = (AdcMirror.ADCRESULT2 * 0.0007326007326f - 1.510850342f) * 29.9f;

    // B1 3PH current -- Inverter IC
    state.I_out_C = (AdcMirror.ADCRESULT3 * 0.0007326007326f - 1.521f) * 3.003834321;
...
//    state.V_cap   = (AdcMirror.ADCRESULT6 * 0.0007326007326f);
    ...
    // judge load state
    RMS_Run(&rms_I_load, state.I_out_A);
    if(rms_I_load.sample_rms > 0.5f)
    {
        state.flag.bits.FLAG_LOAD_STATE = LOAD_ON;
    }
    else
    {
        state.flag.bits.FLAG_LOAD_STATE = LOAD_OFF;
    }
}
...
interrupt void ADC_int_isr(void)
{
    ...
    {
        // PR Voltage loop
        if(state.flag.bits.FLAG_LOAD_STATE == LOAD_OFF)
        {
            state.V_set_load = state.V_set_output * 0.995698436;
        }
        else
        {
            state.V_set_load = state.V_set_output;
        }
        //Phase A
        PR_Run(&pr_vol_A, state.V_set_load * SQUARE_3_INV * state.ref_a, state.V_A, -state.I_out_A);
        //Phase C
        PR_Run(&pr_vol_C, state.V_set_load * SQUARE_3_INV * state.ref_c, state.V_C, -state.I_out_C);
    ...
    else
    {
        //PR Current loop
        //Phase A
        PR_Run(&pr_cur_A, state.I_set_output * state.ref_a, state.I_out_A, 0);
        //Phase C
        PR_Run(&pr_cur_C, state.I_set_output * state.ref_c, state.I_out_C, 0);
    ...
    {
        // force RECTIFIER to LOW
        EPWM_AQ_Rectifier_SoftForce_LOW();

        // force INVERTER into VOLTAGE mode
        state.flag.bits.FLAG_INVERTER_MODE = MODE_VOLTAGE;

...
//    state.Flag_Output_Mode = MODE_CURRENT;
    ...
    else if(state.Flag_Output_Mode == MODE_VOLTAGE)
    {
        ...
        {
            state.V_softstart = state.V_softstart + 0.000001f;
        }
        ...
        //Phase A
        PR_Run(&pr_vol_A, state.pr_V_set * SQUARE_2 * sinf(ramp.ref_angle), state.V_A, 0);
        //Phase C
        PR_Run(&pr_vol_C, state.pr_V_set * SQUARE_2 * sinf(ramp.ref_angle+0.666667*PI), state.V_C, 0);
...
//    UART_VOFA_3Ch_ShowFloat(state.I_set_output * SQUARE_2 * sinf(ramp.ref_angle), ramp.ref_angle, ramp.ref_angle);

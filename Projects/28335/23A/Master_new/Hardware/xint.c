...
interrupt void XINT4_isr(void)
{
    switch(xint_select)
    {
        case SELECT_VOL :
            state.V_set_rms += 0.1;
            if(state.V_set_rms > 29)
            {
                state.V_set_rms = 19;
            }
        break;
        case SELECT_I_TOTAL :
            state.I_total_set_rms += 0.1;
            if(state.I_total_set_rms > 4)
            {
                state.I_total_set_rms = 2;//题目要求2A-4A，但实验过程初值设为1，所以此处也暂设为1
            }
            state.I_set_rms = state.I_distribute_rate * state.I_total_set_rms /(1 + state.I_distribute_rate);
//            state.I_set_rms = state.I_total_set_rms /(1 + state.I_distribute_rate);
        break;
        case SELECT_K_I :
            state.I_distribute_rate += 0.1;
            if(state.I_distribute_rate > 2)
            {
                state.I_distribute_rate = 0.5;
            }
            state.I_set_rms = state.I_distribute_rate * state.I_total_set_rms /(1 + state.I_distribute_rate);
//            state.I_set_rms = state.I_total_set_rms /(1 + state.I_distribute_rate);
        break;
...
interrupt void XINT5_isr(void)
{
    switch(xint_select)
    {
        ...
        case SELECT_I_TOTAL :
            state.I_total_set_rms -= 0.1;
            if(state.I_total_set_rms < 2)//题目要求2A-4A，但实验过程初值设为1，所以此处也暂设为1
            {
                state.I_total_set_rms = 4;
            }
        break;
        case SELECT_K_I :
            state.I_distribute_rate -= 0.1;
            if(state.I_distribute_rate < 0.5)
            {
                state.I_distribute_rate = 2;
            }
        break;
        default:
        break;

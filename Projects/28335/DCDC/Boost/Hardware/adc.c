...
void ADC_Get_Value(void)
{
    ...
    state.I_output = (AdcMirror.ADCRESULT1 * 0.0007326007326f - 1.498) * 4.166667f;
    //A1 DC input voltage
    state.V_input  = AdcMirror.ADCRESULT2 * 0.0007326007326f * 31.0f;
    //A1 DC output current
    state.I_input  = (AdcMirror.ADCRESULT3 * 0.0007326007326f - 1.498) * 4.166667f;
}
...
interrupt void ADC_int_isr(void)
{
    ...
    PID_Run(&pid_vol, state.V_output, state.pi_V_GiveValue, 0);

    //modulate
    duty = (pid_vol.voltage - state.V_input) /  pid_vol.voltage;

    if(duty < 0.333333f)
    {
        duty = 0.333333f;
    }
    else if(duty > 0.714286f)
    {
        duty = 0.714286f;
    }

    EPwm1Regs.CMPA.half.CMPA = 1875 * 0.95f;//
    EPwm2Regs.CMPA.half.CMPA = 1875 * 0.95f * duty;

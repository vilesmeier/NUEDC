...
{         
    ...
    {       
        ...
        LCD_ShowChar(x, y, *p, pen_color, back_color);
        x += 8;
        p++;
    }  
}
...
void LCD_ShowBin(unsigned short x, unsigned short y, unsigned short width, unsigned short height,const int num,int Length, unsigned short pen_color, unsigned short back_color)
{
    ...
    for (i = 0; i < Length; i++)
    {
        LCD_ShowChar(x, y + i * 16, num / LCD_Pow(2, Length - i - 1) % 2 + '0',pen_color,back_color);
    }
}
...
void LCD_ShowFloat(unsigned short x, unsigned short y,const float Number, unsigned char Length,unsigned char Flength,unsigned short pen_color, unsigned short back_color)
{
    ...
    if (Number >= 0)
    {
        LCD_ShowChar(x+(flag-1)*8,y,'+',pen_color, back_color);
        Number1 = Number;
    }
    else
    {
    ...
    //将浮点数转换成整数然后显示
    Number2 = (unsigned int)(Number1 * LCD_Pow(10,Flength));

    for (i = Length; i > 0; i--)
    {
        if(i == (Length - Flength))
        {
            LCD_ShowChar(x+(i+flag)*8,y,'.',pen_color, back_color);
            flag = 0;
            LCD_ShowChar(x+(i+flag)*8,y,Number2 / LCD_Pow(10,Length-i) % 10 + '0', pen_color, back_color);
        }
        else
        {
            LCD_ShowChar(x+(i+flag)*8,y ,Number2 / LCD_Pow(10, Length-i) % 10 + '0', pen_color, back_color);

...
/**
 * @brief 调用该库方法
 * main初始化，指定锁相基频（Hz）、中断频率（Hz）、低通滤波器参数
 * 中断内调用Run函数，输入结构体指针、采样实际值（V/A）
 */
...
//! \return None
//!
static inline void SPLL_1PH_SOGI_Init(SPLL_1PH_SOGI *spll_obj)
{
...
//! \return None
//!
static inline void SPLL_1PH_SOGI_Set(SPLL_1PH_SOGI *spll_obj,
                         float32_t acFreq,

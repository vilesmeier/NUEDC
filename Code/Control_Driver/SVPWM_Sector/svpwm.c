...
void SVPWM_Set(SVPWM* svpwm, Uint16 tbprd, float Udc ,float f_isr)
{
    svpwm->TBPRD = tbprd;
    svpwm->Ts = (1.0f) / f_isr;
    svpwm->k = 1.732051f * svpwm->Ts / Udc;
}
...
{
    //get comp value
    svpwm->comp1 = (svpwm->duty1) * svpwm->TBPRD;
    svpwm->comp2 = (svpwm->duty2) * svpwm->TBPRD;
    svpwm->comp3 = (svpwm->duty3) * svpwm->TBPRD;
...
void SVPWM_Run(SVPWM* svpwm, float Ualpha, float Ubeta)
{
    ...
    switch(svpwm->Sector)
    {
        ...
        case 1: //Sector 2  110 010
            svpwm->t6 = -svpwm->u2 * svpwm->k;
            svpwm->t2 = -svpwm->u3 * svpwm->k;
            if((svpwm->t6 + svpwm->t2) > svpwm->Ts)
            {
                svpwm->Mod_Rate = svpwm->Ts/(svpwm->t2 + svpwm->t6);
                svpwm->t2 = svpwm->Mod_Rate * svpwm->t2;
                svpwm->t6 = svpwm->Mod_Rate * svpwm->t6;
            }
            svpwm->duty2 = 0.25f * (svpwm->Ts - svpwm->t2 - svpwm->t6);
            svpwm->duty1 = svpwm->duty2 + 0.5f * svpwm->t6;
            svpwm->duty3 = svpwm->duty1 + 0.5f * svpwm->t2;
            break;

        case 2: //Sector 6  101 100
            svpwm->t5 = -svpwm->u3 * svpwm->k;
            svpwm->t4 = -svpwm->u1 * svpwm->k;
            if((svpwm->t5 + svpwm-> t4) > svpwm->Ts)
            {
                svpwm->Mod_Rate = svpwm->Ts/(svpwm->t5 + svpwm-> t4);
                svpwm->t5 = svpwm->Mod_Rate * svpwm->t5;
                svpwm->t4 = svpwm->Mod_Rate * svpwm->t4;
            }
            svpwm->duty1 = 0.25f * (svpwm->Ts - svpwm->t4 - svpwm->t5);
            svpwm->duty3 = svpwm->duty1 + 0.5f * svpwm->t5;
            svpwm->duty2 = svpwm->duty3 + 0.5f * svpwm->t4;
            break;

        case 3: //Sector 1  100 110
            svpwm->t4 = svpwm->u2 * svpwm->k;
            svpwm->t6 = svpwm->u1 * svpwm->k;
            if((svpwm->t4 + svpwm->t6) > svpwm->Ts)
            {
                svpwm->Mod_Rate = svpwm->Ts/(svpwm->t4 + svpwm->t6);
                svpwm->t4 = svpwm->Mod_Rate * svpwm->t4;
                svpwm->t6 = svpwm->Mod_Rate * svpwm->t6;
            }
            svpwm->duty1 = 0.25f *(svpwm->Ts - svpwm->t4 - svpwm->t6);
            svpwm->duty2 = svpwm->duty1 + 0.5f * svpwm->t4;
            svpwm->duty3 = svpwm->duty2 + 0.5f * svpwm->t6;
            break;

        case 4: //sector 4  011 001
            svpwm->t3 = -svpwm->u1 * svpwm->k;
            svpwm->t1 = -svpwm->u2 * svpwm->k;
            if((svpwm->t3 + svpwm->t1) > svpwm->Ts)
            {
                svpwm->Mod_Rate = svpwm->Ts/(svpwm->t1 + svpwm->t3);
                svpwm->t1 = svpwm->Mod_Rate * svpwm->t1;
                svpwm->t3 = svpwm->Mod_Rate * svpwm->t3;
            }
            svpwm->duty3 = 0.25f * (svpwm->Ts - svpwm->t1 - svpwm->t3);
            svpwm->duty2 = svpwm->duty3 + 0.5f * svpwm->t3;
            svpwm->duty1 = svpwm->duty2 + 0.5f * svpwm->t1;
            break;

        case 5: //Sector 3  010 011
            svpwm->t2 = svpwm->u1 * svpwm->k;
            svpwm->t3 = svpwm->u3 * svpwm->k;
            if((svpwm->t2 + svpwm->t3) > svpwm->Ts)
            {
                svpwm->Mod_Rate = svpwm->Ts/(svpwm->t2 + svpwm->t3);
                svpwm->t2 = svpwm->Mod_Rate * svpwm->t2;
                svpwm->t3 = svpwm->Mod_Rate * svpwm->t3;
            }
            svpwm->duty2 = 0.25f * (svpwm->Ts - svpwm->t2 - svpwm->t3);
            svpwm->duty3 = svpwm->duty2 + 0.5f * svpwm->t2;
            svpwm->duty1 = svpwm->duty3 + 0.5f * svpwm->t3;
            break;

        case 6: //Sector 5  001 101
            svpwm->t1 = svpwm->u3 * svpwm->k;
            svpwm->t5 = svpwm->u2 * svpwm->k;
            if((svpwm->t5 + svpwm->t1) > svpwm->Ts)
            {
                svpwm->Mod_Rate = svpwm->Ts/(svpwm->t1 + svpwm->t5);
                svpwm->t1 = svpwm->Mod_Rate * svpwm->t1;
                svpwm->t5 = svpwm->Mod_Rate * svpwm->t5;
            }
            svpwm->duty3 = 0.25f * (svpwm->Ts - svpwm->t1 - svpwm->t5);

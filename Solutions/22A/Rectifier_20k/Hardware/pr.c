 ...
 * @param Omega_c   Bandwidth in rad/s
 * @param Omega_0   centeral frequency
...
void PR_Set(PR* pr, float Kp, float Kr, float Omega_c, float Omega_0,float max,float isr_freq)
{
    ...
    pr->A1 = (2 * pr->Omega_0 * pr->Omega_0 * pr->T_isr * pr->T_isr - 8) / pr->A0;
    pr->A2 = (4 - 4 * pr->T_isr * pr->Omega_c + pr->Omega_0 * pr->Omega_0 * pr->T_isr * pr->T_isr) / pr->A0;
    pr->B0 = (4 * pr->Kp + 4 * pr->T_isr * (pr->Kp + pr->Kr) * pr->Omega_c + pr->Kp * pr->Omega_0 * pr->Omega_0 * pr->T_isr * pr->T_isr) / pr->A0;
    pr->B1 = (pr->Kp * (2 * pr->Omega_0 * pr->Omega_0 * pr->T_isr * pr->T_isr - 8)) / pr->A0;
    pr->B2 = (4 * pr->Kp - 4 * pr->T_isr * (pr->Kp + pr->Kr) * pr->Omega_c + pr->Kp * pr->Omega_0 * pr->Omega_0 * pr->T_isr * pr->T_isr) / pr->A0;
    pr->max = max;
    pr->max_inv = 1 / pr->max;
}
...
void PR_Run(PR* pr, float target, float actual, float feedforward)
{
    ...
    //transfer function
    pr->nolimit = pr->B0 * pr->err + pr->B1 * pr->e_last_1 + pr->B2 * pr->e_last_2 - pr->A1 * pr->Res_last_1 - pr->A2 * pr->Res_last_2;

...
interrupt void ADC_int_isr(void)
{
    ...
    //Phase B
    PR_Run(&pr_vol_B, state.V_set_output * SQUARE_2 * sinf(ramp.ref_angle-0.666667*PI), state.V_B, 0);
    //Phase C
    PR_Run(&pr_vol_C, state.V_set_output * SQUARE_2 * sinf(ramp.ref_angle+0.666667*PI), state.V_C, 0);
    //Modulate
    EPwm1Regs.CMPA.half.CMPA = (pr_vol_A.out + 1.0f) * 1875;
    EPwm2Regs.CMPA.half.CMPA = (pr_vol_B.out + 1.0f) * 1875;
    EPwm3Regs.CMPA.half.CMPA = (pr_vol_C.out + 1.0f) * 1875;

    //Reference Angle Generate
    Ramp_Run(&ramp);

    UART_VOFA_3Ch_ShowFloat(state.V_A, state.V_B, state.V_C);

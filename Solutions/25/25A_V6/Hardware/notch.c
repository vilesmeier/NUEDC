...
void NOTCH_Set(NOTCH* notch, float fstop, float fsample,float mode)
{
    ...
    else
    {
        ...
        //Numerator
        notch->A2 = 100 * notch->delta_t;
        notch->A1 = 0;
        notch->A0 = -100 * notch->delta_t;

        //Denominator
        notch->B2 = 4 + 100 * notch->delta_t + notch->omega_0 * notch->omega_0 * notch->delta_t * notch->delta_t;
        notch->B1 = -8 + 2 * notch->omega_0 * notch->omega_0 * notch->delta_t * notch->delta_t;
        notch->B0 = 4 - 100 * notch->delta_t + notch->omega_0 * notch->omega_0 * notch->delta_t * notch->delta_t;
    }
}

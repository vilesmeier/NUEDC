...
//    memcpy(&RamfuncsRunStart, &RamfuncsLoadStart, (Uint32)&RamfuncsLoadSize);
    ...
    //PR
    PR_Init(&pr_vol_A);
    PR_Set(&pr_vol_A,0.1f,50.0f,1.5*2*PI,100*PI,VDC,20000);
    PR_Init(&pr_vol_C);
    PR_Set(&pr_vol_C,0.1f,50.0f,1.5*2*PI,100*PI,VDC,20000);

    PR_Init(&pr_rec_a);
    PR_Set(&pr_rec_a,0.0000001f,30.0f,2*PI,100*PI,VDC,20000);
    PR_Init(&pr_rec_c);
    PR_Set(&pr_rec_c,0.0000001f,30.0f,2*PI,100*PI,VDC,20000);
...
//        }
        ...
        // vofa debug
        UART_VOFA_3Ch_ShowFloat(state.I_In_a, state.I_In_c, pr_vol_C.out);

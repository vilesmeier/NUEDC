...
void ADC_Get_Value(void)
{
    ...
    state.V_ab    = (AdcMirror.ADCRESULT0 * 0.0007326007326f - 1.498647619f) * 30.1f;

    // B0 3PH current -- Inverter IA
    state.I_out_A = (AdcMirror.ADCRESULT1 * 0.0007326007326f - 1.50627f) * 4.149549774f;

    // A1 3PH voltage -- Inverter Ubc
    state.V_bc    = (AdcMirror.ADCRESULT2 * 0.0007326007326f - 1.506980952f) * 30.1f;

    // B1 3PH current -- Inverter IC
    state.I_out_C = (AdcMirror.ADCRESULT3 * 0.0007326007326f - 1.500183151f) * 4.231381197f;
    ...
    state.I_In_c  = (AdcMirror.ADCRESULT5 * 0.0007326007326f - 1.501127763f) * 4.234179105f;
    ...
    state.I_In_a  = (AdcMirror.ADCRESULT7 * 0.0007326007326f - 1.500890228f) * 4.177970079f;
...
//    EPwm1Regs.CMPA.half.CMPA = 1875 * (sinf(ramp.ref_angle) + 1.0f);
//    EPwm2Regs.CMPA.half.CMPA = 1875 * (sinf(ramp.ref_angle-0.666667*PI) + 1.0f);
//    EPwm3Regs.CMPA.half.CMPA = 1875 * (sinf(ramp.ref_angle+0.666667*PI) + 1.0f);

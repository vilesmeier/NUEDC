...
/**
 * @brief This function must be called after RMS_Run or RMS_Push !!!
...
float RMS_Delay(RMS *rms_addr, float phase)
{
    ...
    if(rms_addr->index < (index_differ + 1))
    {
        return rms_addr->data[400 + rms_addr->index - index_differ - 1];
    }
    else
    {
        return rms_addr->data[rms_addr->index - index_differ - 1];
    }
}
 ...
 * 
 * @param rms_addr  struct pointer
 * @return float    current cycle input data
 */
float RMS_Current_Value(RMS *rms_addr)
{
    if(rms_addr->index == 0)
    {
        return rms_addr->data[399];
    }
    else
    {
        return rms_addr->data[rms_addr->index - 1];
    }
}
